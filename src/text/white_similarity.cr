...
# Crystal implementation of the string similarity described by Simon White
...
module Text
  class WhiteSimilarity
    ...
    def initialize
      @word_letter_pairs = Hash(::String, Array(::String)).new
    end
    ...
    private def word_letter_pairs(str)
      @word_letter_pairs[str] ||= str.upcase.split(/\s+/).map do |word|
        (0 ... (word.size - 1)).map { |i| word[i, 2] }
      end.flatten
    end
  end
end