# Crystal implementation of the Double Metaphone algorithm by Lawrence Philips,
...
module Text
  module Metaphone
    ...
    def double_metaphone(str)
      primary, secondary, current = [] of ::String, [] of ::String, 0
      original, length, last = "#{str}     ".upcase, str.size, str.size - 1
      ...
      while primary.size < 4 || secondary.size < 4
        break if current > str.size
    ...
    private def slavo_germanic?(str)
      /W|K|CZ|WITZ/ =~ str
    end

    private def vowel?(str)
      /^A|E|I|O|U|Y$/ =~ str
    end

    private def double_metaphone_lookup(str, pos, length, last) : Tuple((::String | Nil), (::String | Nil), Int32)
      case str[pos, 1]
        when /^A|E|I|O|U|Y$/
          if pos == 0
            return "A", "A", 1
          ...
          if pos > 1 &&
            ...
            if pos > 0 && "CHAE" == str[pos, 4]
              return "K", "X", 2
                          ...
                          ["HARAC", "HARIS"].includes?(str[pos + 1, 5]) ||
                          ["HOR", "HYM", "HIA", "HEM"].includes?(str[pos + 1, 3])
                        ) && str[0, 5] != "CHORE")
              return "K", "K", 2
            elsif (["VAN ","VON "].includes?(str[0, 4]) ||
                            "SCH" == str[0, 3] ||
                            ["ORCHES","ARCHIT","ORCHID"].includes?(str[pos - 2, 6]) ||
                            ["T","S"].includes?(str[pos + 2, 1]) || (
                              ((0 == pos) || ["A","O","U","E"].includes?(str[pos - 1, 1])) &&
                              ["L","R","N","M","B","H","F","V","W"," "].includes?(str[pos + 2, 1])
                            ))
              return "K", "K", 2
            elsif pos > 0
            ...
            if /^ (C|Q|G)$/ =~ str[pos + 1, 2]
              ...
              return "K", "K", (/^C|K|Q$/ =~ str[pos + 1, 1] && !(["CE","CI"].includes?(str[pos + 1, 2])) ? 2 : 1)
            end
          end
        when "D"
          if "DG" == str[pos, 2]
